...
String get_detailed_info(String point) {
    return String(";") +
           IP + String(";") +
           MAC + String(";") +
           SN_Utils__TimestampToStr(SN_Utils__GetTimestamp()) + String(";") +
           point + ";";
}
...
void SN_UART_Init() {
    Serial.begin(115200);
    SN_Logger_Log(true, "SN_UART_Init", "Serial port initialized");
}
...
void SN_Logger_Log(bool serial_verbose, String point, const char *format, ...) {
    ...
    if (serial_verbose) {
        // Serial.println(log_message_full);
